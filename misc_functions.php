...
$script = <<< EOF
	...
	image.src = "http://ktteradio.com/wp-content/themes/LiveTheme_223/images/video-placeholder.jpg";

	httpGetVideoAsync($video_id, image, 
		
		function callback(jsonText, newImage){
			if(jsonText){
				var json = JSON.parse(jsonText);
				newImage.src = json.thumbnail_medium;				
			}else{
				console.log("Problem with the Response is "+ jsonText);				
			}
		}
	) 
   </script>
...
function wp_vid_lightbox_enqueue_script()
{
    ...
    if(get_option('wpvl_enable_prettyPhoto')=='1')
    {
        ...
        wp_enqueue_script('async.js', WP_VID_LIGHTBOX_URL.'/js/async.js', false, '1.0' );
